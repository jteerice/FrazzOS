...
.PHONY: run-dump
run-dump: $(IMAGE_NAME).iso
	qemu-system-x86_64 -no-reboot -d int -M q35 -m 2G -smp 4 -cdrom $(IMAGE_NAME).iso -boot d
...
run-hdd: $(IMAGE_NAME).hdd
	qemu-system-x86_64 -M q35 -m 2G -smp 4 -hda $(IMAGE_NAME).hdd

.PHONY: run-hdd-uefi
run-hdd-uefi: ovmf $(IMAGE_NAME).hdd
	qemu-system-x86_64 -M q35 -m 2G -smp 4 -bios ovmf/OVMF.fd -hda $(IMAGE_NAME).hdd
